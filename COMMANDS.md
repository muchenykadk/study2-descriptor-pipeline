...
**Re-derive the design factors** after editing `env/design_factors.json` — rewrites the
records and the inventory without recomputing geometry or calling the API:
```powershell
python 03_src/refresh_factors.py --dry-run
python 03_src/refresh_factors.py
```
The per-fragment HTML reports are regenerated by the pipeline, not by this script.
